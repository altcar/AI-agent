...
if daytona_config.api_key:
    ...
    daytona = None
    logger.warning("Daytona client not initialized; API key is missing")


def _require_daytona() -> Daytona:
    if daytona is None:
        raise RuntimeError(
            "Daytona API key is missing. Set daytona.daytona_api_key in config.toml or disable sandbox tools."
        )
    return daytona
...
async def get_or_start_sandbox(sandbox_id: str):
    ...
    try:
        sandbox = _require_daytona().get(sandbox_id)
        ...
        if (
            ...
            try:
                _require_daytona().start(sandbox)
...
def create_sandbox(password: str, project_id: str = None):
    ...
    # Create the sandbox
    sandbox = _require_daytona().create(params)
...
async def delete_sandbox(sandbox_id: str):
    ...
    try:
        ...
        # Delete the sandbox
        _require_daytona().delete(sandbox)
